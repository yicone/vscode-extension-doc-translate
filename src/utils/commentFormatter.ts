...
export function getCommentFormat(languageId: string): CommentFormat {
    switch (languageId) {
        ...
        default:
            logger.warn('Unknown language: ' + languageId + ', using default comment format');
            return {
                docstringOpen: '/*',
                docstringClose: ' */',
                docstringLinePrefix: ' * ',
 ...
 */
export function formatDocstring(translation: string, languageId: string, indentation: string = ''): string[] {
    const format = getCommentFormat(languageId);
    const lines = translation.split('\n');

    if (lines.length === 1) {
        // Single line docstring
        return [indentation + format.docstringOpen + lines[0] + format.docstringClose];
    }
